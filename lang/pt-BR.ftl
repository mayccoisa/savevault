...
logs-column-when = Quando
logs-column-game = Jogo
logs-column-backup = Backup
logs-column-change = O que mudou
logs-column-files = Arquivos
logs-column-size = Tamanho
logs-kind-full = Completo
logs-kind-differential = Diferencial
logs-no-change = Sem mudança
logs-added = {$count} novos
logs-changed = {$count} alterados
logs-removed = {$count} removidos
logs-empty = Nenhum backup foi feito ainda, então não há o que registrar.
vault-empty = Nenhum jogo tem backup aqui ainda.
vault-stored-games = { $count } jogos guardados, do backup mais antigo para o mais novo
logs-back-to-runs = Todas as execuções
logs-run-detail = Execução de { $when }
...
button-recheck-emulators = Verificar de novo
button-check-for-update = Verificar atualização
emulators-explanation = Aponte a pasta de dados de cada emulador. O backup e a restauração saem dali, mesmo que a pasta mude de lugar.
emulators-summary = { $found } de { $total } encontrados neste computador
emulator-coming-soon = Em breve
emulator-not-checked-yet = Ainda não verificado.
emulator-not-found = { $name } não foi encontrado neste computador.
emulator-using-folder = Usando { $path } ({ $games } arquivo(s) de save).
emulator-state-ready = Encontrado
emulator-state-missing = Não encontrado
emulator-state-no-saves = Sem save
emulator-state-ambiguous = Ambíguo
emulator-state-unchecked = Não verificado
updating-app = Baixando a atualização...
app-is-up-to-date = Você já está na versão mais recente.
app-updated = Atualizado para a versão { $version }. Feche e abra o programa de novo para usar.
app-update-failed = A atualização não deu certo: { $message }
emulator-ambiguous = Foram encontradas { $folders } pastas de dados do { $name }, então o destino fica indefinido. Mantenha só a que você quer.
emulator-folder-empty = Escolha a pasta de dados do emulador.
emulator-folder-missing = Esta pasta não existe.
emulator-folder-wrong = Isto não parece a pasta de dados do { $name }. Falta: { $missing }
emulator-folder-without-saves = Pasta certa, mas não há save nenhum nela. Procurei em: { $areas }. Se os seus jogos estão em outro disco, aponte aquela instalação.
emulator-folder-ready = Pasta reconhecida: { $saves } arquivo(s) de save encontrados.
emulator-using-folder-without-saves = Usando { $path }, mas não há save nenhum nela. Se os seus jogos estão em outro disco, acrescente aquela instalação como pasta abaixo.
