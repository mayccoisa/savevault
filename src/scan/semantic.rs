...
pub enum Unresolved {
    ...
    /// The emulator was found, but the area inside it has a variable segment (the PS3's user
...
mod tests {
    ...
    /// The PS3 case, and the reason a variable segment exists at all.
    ///
    /// The profile id is generated by the emulator, so the machine being restored to almost
    /// certainly has a different one. Reapplying the recorded id would create the source machine's
    /// profile folder: correct-looking on disk, and invisible to the game.
    #[test]
    fn re_anchors_a_ps3_save_onto_this_machines_profile() {
        let install = tempfile::tempdir().unwrap();
        let root = StrictPath::from(install.path().to_path_buf());
        std::fs::create_dir_all(
            root.joined("dev_hdd0/home/00009999/savedata")
                .as_std_path_buf()
                .unwrap(),
        )
        .unwrap();

        let recorded = "D:/PS3/dev_hdd0/home/00000001/savedata";
        let semantics = BackupSemantics {
            directories: btree_map! {
                recorded.to_string(): DirectorySemantics {
                    kind: SemanticDirKind::Emulator { app: App::Rpcs3, area: Area::Saves },
                },
            },
        };

        let target = emulator_restore_target(
            &StrictPath::new(format!("{recorded}/BLES00932-AUTO/SAVEDATA")),
            &semantics,
            &emulator::Roots::for_test(vec![(App::Rpcs3, root.clone())]),
        );

        assert_eq!(
            EmulatorTarget::Redirected(root.joined("dev_hdd0/home/00009999/savedata/BLES00932-AUTO/SAVEDATA")),
            target
        );
    }

    /// Two profiles on the destination machine: which one owns the save is unknown, and guessing
    /// with save data is the one thing this program must not do.
    #[test]
    fn refuses_a_ps3_save_when_the_machine_has_two_profiles() {
        let install = tempfile::tempdir().unwrap();
        let root = StrictPath::from(install.path().to_path_buf());
        for profile in ["00000001", "00000002"] {
            std::fs::create_dir_all(
                root.joined(format!("dev_hdd0/home/{profile}/savedata"))
                    .as_std_path_buf()
                    .unwrap(),
            )
            .unwrap();
        ...
        let target = emulator_restore_target(
            &StrictPath::new(format!("{recorded}/BLES00932-AUTO/SAVEDATA")),
            &semantics,
            &emulator::Roots::for_test(vec![(App::Rpcs3, root)]),
        );
